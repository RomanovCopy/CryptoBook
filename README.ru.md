...
- В редакторе появилась палитра из шести ячеек для применения часто используемых цветов текста одним щелчком.
- Щелчок правой кнопкой по ячейке открывает выбор нового цвета из палитры редактора, не изменяя цвет текущего текста.
- Настроенные цвета сохраняются в параметрах текущего пользователя Windows и восстанавливаются при следующих запусках.
- Названия и подсказки палитры локализованы для английского, русского, немецкого и украинского интерфейсов.

Подробнее — в [описании релиза](docs/releases/v1.1.4.0.md) и [истории изменений](CHANGELOG.md).
