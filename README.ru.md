...
| --- | --- |
| ![Markdown с локальной иллюстрацией](docs/screenshots/markdown-preview.png) | ![Встроенная справка по синтаксису Markdown](docs/screenshots/markdown-help.png) |
| **Режим чтения Sepia** | **Навигация по документам** |
| ![Иллюстрированный документ в теме Sepia](docs/screenshots/sepia-reading.png) | ![Боковое меню и документ с иллюстрацией](docs/screenshots/side-menu.png) |

Скриншоты версии 1.1.3.5 показывают дневник путешествия с иллюстрацией и фоном страницы.
[Описание съёмки и примеры документов](docs/screenshots/README.md).

## Новое в 1.1.3.6

- Пароль для нового шифрования: 8–128 символов и не менее шести различных символов без учёта регистра; распространённые и повторяющиеся пароли отклоняются. Существующие файлы открываются с прежними паролями.
- Аварийные копии защищённых документов теперь используют пароль, Argon2id и AES-256-GCM; копии обычных документов по-прежнему защищаются Windows DPAPI.
- В настройках доступны **«Сбросить ключ сейчас»**, состояние ключа и подсказка **Ctrl+L**. Сброс сохраняет открытые обычные документы и их несохранённые изменения.
- Усилены защита пароля в памяти и обработка ошибок записи снимка при сбросе ключа.
- Удалены запрос восстановления при запуске и баннер восстановления. Существующие снимки сохраняются, но восстановление через баннер главного окна больше не предлагается.

Подробнее — в [описании релиза](docs/releases/v1.1.3.6.md), [истории изменений](CHANGELOG.md) и [описании защиты](docs/security-hardening.md).
