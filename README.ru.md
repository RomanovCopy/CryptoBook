...
- Перемещение фонового изображения через **Ctrl + перетаскивание левой кнопкой мыши**; **Esc** отменяет перемещение.
- Фон ограничен областью бумаги и сохраняет положение при масштабировании, прокрутке и изменении размеров окна.
- При сохранении в XamlPackage или защищённый текстовый документ программа просит подтвердить обрезку.
  После успешного сохранения внутри документа остаётся только выбранная область, а положение фона фиксируется;
  для нового кадрирования нужно выбрать другое изображение. Отмена или ошибка сохранения оставляет фон редактируемым.
- В режиме чтения границы редактора больше не накладываются на документ.
- Обновлены скриншоты редактора, режима чтения и Markdown с демонстрационными материалами.

Подробнее — в [описании релиза](docs/releases/v1.1.3.5.md) и [истории изменений](CHANGELOG.md).
