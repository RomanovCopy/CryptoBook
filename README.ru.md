...
- В настройках можно выбрать запуск со стартовой страницы или сразу с текстового редактора; выбор сохраняется для текущего пользователя Windows.
- При запуске сразу с редактора страница открывается в реальном масштабе 100%, а не растягивается по начальной ширине окна. Масштабирование через Ctrl + колесо мыши по-прежнему сохраняется после изменения размера окна и перезагрузки редактора.
- Значки на панели задач, в меню «Пуск», на рабочем столе и для связанных типов документов теперь берутся из стабильного `CryptoBook.exe`. При обновлении закреплённый значок переносится до удаления устаревших файлов значков с версией в имени.

Подробнее — в [описании релиза](docs/releases/v1.1.3.7.md) и [истории изменений](CHANGELOG.md).
