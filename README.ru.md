...
- В файловом менеджере можно создать защищённые `.cbook`-копии одного или нескольких выбранных файлов, не удаляя и не заменяя исходники.
- Копия сначала записывается во временный файл и проверяется перед публикацией. Существующие файлы не перезаписываются, а при совпадении имён создаются варианты вида `документ (2).cbook`.
- Уже защищённые файлы пропускаются. Для пакетной операции доступны прогресс, отмена и локализованный итоговый отчёт.
- Если задан ключ шифрования, согласие на предложение редактора теперь создаёт `.cbook`-копию с уникальным именем и сохраняет исходный документ без изменений.

Подробнее — в [описании релиза](docs/releases/v1.1.3.8.md) и [истории изменений](CHANGELOG.md).
