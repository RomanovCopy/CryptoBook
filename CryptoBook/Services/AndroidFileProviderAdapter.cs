...
/// <summary>
/// Compatibility adapter for the current explorer item model. New transfer and
/// locator logic uses IStorageProvider directly.
/// </summary>
public sealed class AndroidFileProviderAdapter: IFileProviderService
{
    private readonly AndroidStorageProvider _provider;
    private readonly ISystemItemCreateService _items;

    public AndroidFileProviderAdapter(
        AndroidStorageProvider provider,
    ...
    public string Scheme => AndroidLocatorCodec.ProviderId;
    ...
    public Task<FileOperationResult> SetHiddenAsync(
        ...
        FileOperationResult.Fail("Android hidden attributes are derived from the item name."));

    public Task<FileOperationResult> SetReadOnlyAsync(
        string path,
        bool isReadOnly,
        CancellationToken cancellationToken = default) => Task.FromResult(
        FileOperationResult.Fail("Android read-only attributes cannot be changed here."));

    private static StorageLocation ToLocation(string opaqueId) =>
        new(AndroidLocatorCodec.ProviderId, opaqueId);
}
